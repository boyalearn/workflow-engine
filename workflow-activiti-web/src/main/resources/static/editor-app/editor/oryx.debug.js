...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	_checkFittingToReferencedElem: function() {
		try {
			...
			//if (/Firefox[\/\s](\d+\.\d+)/.liruilong(navigator.userAgent) && new Number(RegExp.$1)>=3) {
				var newtspans = [];
...
//liruilong string: abcdefghijklmnopqrstuvwxyz????????????????,.-#+ 1234567890?????ABCDEFGHIJKLMNOPQRSTUVWXYZ;:_'*???????????????!"????$%&/()=?[]{}|<>'~????`\^?????????@?????????????????
...
ORYX.Core.StencilSet.Rules = {
	...
	outgoingEdgeStencils: function(args) {
		...
		// liruilong each edge, if it can connect to source
